...
class CustomDDIMScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        cur_idx = torch.where(self.timesteps == timestep)[0]
        # print(f"cur_idx: {cur_idx}")
        prev_timestep = self.timesteps[cur_idx + 1][0] if cur_idx + 1 < self.num_inference_steps else timestep
